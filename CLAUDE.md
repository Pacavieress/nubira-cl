...
## Pendiente: /guias y /guias/[cat] (PHP real) tienen overflow horizontal en producción — decisión de ancho del port a Next.js reabierta el 11/09/2026

Detectado el 08/09/2026 auditando la migración a Next.js (`web/`), al verificar por qué el ancho de contenido de `/guias` no calzaba con el resto de páginas de listado tras un fix de padding. Medido con `document.body.scrollWidth` en `nubira.local/guias` a 1440px: **1696px contra un viewport de 1440px** — 256px de contenido real desbordando hacia la derecha, invisible sin hacer scroll horizontal (no se nota en capturas de pantalla de viewport fijo). Mismo bug confirmado en `/guias/[cat]` (ej. `nubira.local/guias/matematicas`). No aparece en mobile (`lg:` no aplica bajo 1024px).

Causa: `app/guias.php` es la única página de este tipo cuyo `<main>` combina `w-full` + `lg:ml-64` + `mx-auto` (lo necesita para el patrón de footer pegado abajo con `flex-grow`). `w-full` fija el ancho al 100% del contenedor padre e ignora el margen para el cálculo de ancho — a diferencia de `busqueda.php`/`clases_servicios.php`/`vitrina_apuntes.php`, que no usan `w-full` en el `<main>` y por eso el margen sí se resta con normalidad. Efecto real: las cards de categoría miden ~442px de ancho en vez de los ~256-357px esperados; pasa desapercibido hoy porque con pocas categorías igual caben dentro del viewport visible.

**Decisión original (08/09/2026)**: el port a Next.js no replicó este bug — se dejó con el ancho "sano" (padding consistente con el resto del sitio, contenedor centrado de 1120px a 1440px, vía `w-full max-w-[1600px] mx-auto ... lg:pl-72`), no con el ancho real y roto de producción.

**Decisión reabierta (11/09/2026)**: `web/src/app/guias/page.tsx` pasó de `w-full max-w-[1600px] mx-auto ... lg:pl-72` a `max-w-[1600px] lg:ml-64 ...` (sin `w-full` ni `mx-auto`) — el mismo fix de ancho ya aplicado en `servicios/page.tsx` y `apuntes/page.tsx`: pegado al sidebar, sin el aire simétrico que dejaba el centrado. Se priorizó consistencia visual entre las vitrinas del sitio por sobre el centrado aislado que tenía guías. Sigue **sin portarse** el combo real de `guias.php` (`w-full` + `lg:ml-64` juntos, que es justamente lo que desborda 256px en producción): el fix de Next mantiene `lg:ml-64` (correcto, igual que en las otras vitrinas) pero deliberadamente omite el `w-full` que causa el bug. El PHP real sigue sin arreglar — pendiente arreglarlo ahí algún día (quitar `w-full` del `<main>` de `guias.php`, o compensar el ancho con `max-w-` explícito) — no urgente, es un defecto invisible en el uso normal.
