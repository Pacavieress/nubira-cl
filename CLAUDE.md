...
## Pendiente: /guias y /guias/[cat] (PHP real) tienen overflow horizontal en producción — no replicado en el port a Next.js
...
**Decisión tomada (08/09/2026)**: el port a Next.js NO replica este bug — se dejó con el ancho "sano" (padding consistente con el resto del sitio, contenedor de 1120px a 1440px), no con el ancho real y roto de producción. Pendiente arreglar en el PHP algún día (quitar `w-full` del `<main>` de `guias.php`, o compensar el ancho con `max-w-` explícito) — no urgente, es un defecto invisible en el uso normal.
