...
// Puerto de app/img_apunte.php + imagen_compartir_apunte.php — SOLO formato POST 4:5
// (mismo criterio de slices que Compartir Desafío: HISTORY y la card de servicios quedan
// para otra pieza). A diferencia de CompartirDesafioModal, acá el "objetivo" (el apunte) ya
// es fijo desde que se abre el modal — no hay paso 1 de "elegir materia", va directo a la
// vista previa. Ver server/src/modules/compartir/ para el motor de generación.
...
export function CompartirApunteModal({
  apunteId,
  titulo,
  abierto,
  onCerrar,
  ...
  const imgUrl = `/api/apunte/compartir/${apunteId}/post`;
  const caption = `${titulo}\n\nEncuéntralo en Nubira: https://nubira.cl/apunte/${apunteId}\n\n#Nubira #Apuntes`;
  const nombreArchivo = `nubira-apunte-${apunteId}.jpg`;

  function track(formato: Formato) {
    fetch("/api/apunte/compartir/track", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ apunteId, formato }),
    }).catch(() => {});
  }
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm" onClick={onCerrar}>
      ...
      >
        <div className="flex items-center justify-between px-5 pt-4 pb-2">
          <h3 className="text-base font-bold text-gray-900 tracking-tight">Comparte este apunte</h3>
