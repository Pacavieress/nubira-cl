...
$stmt = $conn->prepare("UPDATE contratos SET estado = 'liberado', fecha_cierre = NOW() WHERE id = ? AND estado = 'en_progreso'");
$stmt->bind_param("i", $id);

if ($stmt->execute()) {
    // CORRECCIÓN: Redirección a la ruta correcta en /app/
    header("Location: /app/admin_contratos.php?msg=liberado_ok&estado=en_progreso");
} else {
