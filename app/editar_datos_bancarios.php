    ...
    <div class="bg-white border border-slate-100 rounded-3xl p-6 md:p-8">
        <form method="POST" class="space-y-6" id="banco-form">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
                ...
                <div class="space-y-2">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">Institución Bancaria</label>
                    <div class="relative">
                        <select name="banco" required class="w-full border border-slate-200 bg-slate-50 px-4 py-3.5 rounded-2xl focus:ring-0 focus:border-[#54A6D8] focus:bg-white text-slate-800 font-medium transition-colors cursor-pointer outline-none">
                ...
                <div class="space-y-2">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">Tipo de Cuenta</label>
                    <div class="relative">
                        <select name="tipo_cuenta" required class="w-full border border-slate-200 bg-slate-50 px-4 py-3.5 rounded-2xl focus:ring-0 focus:border-[#54A6D8] focus:bg-white text-slate-800 font-medium transition-colors cursor-pointer outline-none">
                            <?php
                ...
                <div class="space-y-2">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">Número de Cuenta</label>
                    <input type="text" name="numero_cuenta" value="<?= htmlspecialchars($datos['numero_cuenta'] ?? '') ?>" required
                           placeholder="Ej: 123456789"
                           class="w-full border border-slate-200 bg-slate-50 px-4 py-3.5 rounded-2xl focus:ring-0 focus:border-[#54A6D8] focus:bg-white text-slate-800 font-medium transition-colors placeholder-slate-300 outline-none"
                           oninput="this.value=this.value.replace(/[^0-9]/g,'')">
                    <p class="text-[10px] font-medium text-slate-400 pl-1 mt-1">Solo números, sin guiones.</p>
                </div>

                <div class="space-y-2">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">Nombre del Titular</label>
                ...
                <div class="space-y-2 md:col-span-2">
                    <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-widest pl-1">RUT del Titular</label>
                    <input type="text" name="rut" value="<?= htmlspecialchars($datos['rut'] ?? '') ?>" required
                           placeholder="Ej: 12345678-9"
                           class="w-full md:w-1/2 border border-slate-200 bg-slate-50 px-4 py-3.5 rounded-2xl focus:ring-0 focus:border-[#54A6D8] focus:bg-white text-slate-800 font-medium transition-colors placeholder-slate-300 uppercase outline-none"
                           oninput="this.value=this.value.replace(/[^0-9kK-]/g,'')">
                    <p class="text-[10px] font-medium text-slate-400 pl-1 mt-1">Debe coincidir exactamente con el titular del banco.</p>
                </div>

            </div>

            <div class="pt-6 mt-8 border-t border-slate-100 flex items-center justify-between">
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 hidden sm:flex">
                    <i class="fa-solid fa-shield-halved"></i> Conexión Segura
                </span>
                <button type="submit" class="w-full sm:w-auto bg-[#54A6D8] text-white active:bg-blue-600 md:hover:bg-[#4392c3] font-bold py-3.5 px-8 rounded-2xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 text-sm shadow-none border border-transparent">
