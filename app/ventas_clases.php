...
<div id="top-acciones" class="fixed top-0 left-0 w-full bg-white border-b border-slate-200 z-[70] transform -translate-y-full transition-transform duration-300 flex items-center justify-between px-4 py-3 md:pl-72 md:pr-8 h-[60px]">
    <div class="flex items-center gap-3">
        <button onclick="cancelarSeleccion()" class="w-8 h-8 flex items-center justify-center text-slate-500 hover:bg-slate-100 rounded-full transition-colors active:scale-95">
    ...
    <button onclick="eliminarSeleccionados()" class="text-red-500 hover:text-red-700 font-bold text-sm px-3 py-1.5 rounded-full active:bg-red-50 transition-colors flex items-center gap-1.5">
  ...
  <div class="w-full">
    ...
    <div class="sticky top-16 bg-white/95 backdrop-blur-sm z-30 border-b border-slate-200 px-4 md:px-6 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            ...
            <button onclick="exportarCSV()" class="inline-flex items-center justify-center gap-1.5 bg-emerald-50 text-emerald-700 font-bold py-1.5 px-3 rounded-xl hover:bg-emerald-100 active:scale-95 transition text-[11px] tracking-wide">
                <i class="fa-solid fa-file-csv"></i> Exportar
            </button>
            <?php endif; ?>
            <a href="/datos_bancarios" class="inline-flex items-center justify-center gap-1.5 bg-slate-100 text-slate-700 font-bold py-1.5 px-3 rounded-xl hover:bg-slate-200 active:scale-95 transition text-[11px] tracking-wide">
   ...
   <div class="md:px-6 pt-2">
        <?php if (!empty($ventasAgrupadas)): ?>
            <div id="lista-ventas" class="pb-10 space-y-4 mt-2">
                ...
                <div class="group-dia" id="dia-<?= $dia ?>">
                    ...
                    <button onclick="toggleGrupo('<?= $idGrupo ?>', 'icono-<?= $idGrupo ?>')" class="w-full px-4 md:px-2 py-2 flex items-center justify-between bg-[#f8fafc] transition-colors cursor-pointer sticky top-[108px] sm:top-[115px] z-20">
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-slate-500 text-[12px] uppercase tracking-widest"><?= $etiquetaDia ?></span>
                            <span class="bg-slate-200/70 text-slate-600 text-[9px] font-bold px-1.5 py-0.5 rounded-md"><?= $cantidadDia ?></span>
                        </div>
                        <i id="icono-<?= $idGrupo ?>" class="fa-solid fa-chevron-down text-slate-400 text-[10px] transition-transform duration-200"></i>
                    </button>

                    <div id="<?= $idGrupo ?>" class="hidden bg-white border-y md:border border-slate-200 md:rounded-2xl overflow-hidden">
                        <ul class="divide-y divide-slate-100">
                            <?php foreach ($clasesDelDia as $s): 
                                ...
                                if ($is_liberado) {
                                    $estado_color = 'text-emerald-500 bg-emerald-50';
                                    $texto_estado = 'Terminada';
                                } elseif ($is_en_curso) {
                                    $estado_color = 'text-blue-500 bg-blue-50';
                                    $texto_estado = 'En Curso';
                                } elseif ($is_cancelado) {
                                    $estado_color = 'text-red-500 bg-red-50';
                                    $texto_estado = 'Cancelada';
                                } else {
                                    $estado_color = 'text-amber-500 bg-amber-50';
                            ...
                            <li class="relative group row-item" id="venta-<?= $s['id_contrato'] ?>" data-id="<?= $s['id_contrato'] ?>">
                                ...
                                <div class="flex items-center justify-between p-4 md:px-4 hover:bg-slate-50 transition-colors gap-3 cursor-pointer"
                                    ...
                                    <div class="flex items-center gap-3 flex-1 min-w-0 z-20">
                                        <div class="chk-ui-container hidden shrink-0" id="chk-ui-<?= $s['id_contrato'] ?>" onclick="event.stopPropagation(); toggleSeleccion(<?= $s['id_contrato'] ?>)">
                                            <div class="w-5 h-5 rounded-full border-2 border-slate-300 flex items-center justify-center transition-colors bg-white">
                                                <i class="fa-solid fa-check text-[10px] text-white opacity-0 transition-opacity"></i>
                                            </div>
                                        </div>

                                        <div class="w-12 h-12 rounded-xl bg-slate-100 overflow-hidden shrink-0 border border-slate-200 relative">
                                            <img src="<?= htmlspecialchars($rutaImg) ?>" onerror="this.src='/img/portadas/servicios/clases.webp';" class="w-full h-full object-cover">
                                        </div>

                                        <div class="flex-1 min-w-0">
                                            <h3 class="font-bold text-slate-800 text-[14px] line-clamp-1 leading-tight mb-0.5"><?= htmlspecialchars($s['titulo']) ?></h3>
                                            <div class="flex items-center gap-1.5 text-[11px] text-slate-400 font-medium truncate">
                                                <div class="w-4 h-4 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center text-[7px] font-bold shrink-0"><?= $inicial_alumno ?></div>
                                                <span class="truncate"><?= formatearNombrePrivado($s['comprador_nombre']) ?></span>
                                                <span>•</span>
                                                <span class="font-mono text-slate-400"><?= date('H:i', strtotime($s['fecha_creacion'])) ?></span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex flex-col items-end gap-1.5 shrink-0 pl-2 z-20">
                                        ...
                                        <div class="flex justify-end" onclick="event.stopPropagation()">
                                            ...
                                            <?php elseif ($is_liberado && !$ya_calificado): ?>
                                                <a href="/app/evaluar_servicio.php?id=<?= $s['id_contrato'] ?>" class="inline-flex items-center justify-center gap-1.5 bg-[#54A6D8]/10 text-[#54A6D8] hover:bg-[#54A6D8]/20 text-[10px] font-bold px-2 py-1 rounded-md transition-colors">
                                                    <i class="fa-solid fa-star"></i> Nota
                                                </a>
                                            <?php elseif ($ya_calificado): ?>
                                                <span class="text-[9px] font-extrabold text-emerald-500 uppercase tracking-wider">
                                                    Listo
                                                </span>
                                            <?php else: ?>
                                                <span class="text-[9px] font-extrabold <?= $estado_color ?> uppercase tracking-wider">
                                                    <?= $texto_estado ?>
                                                </span>
                                            <?php endif; ?>
                                        </div>
        ...
        <?php else: ?>
            <div class="bg-white p-8 text-center border-y md:border border-slate-200 md:rounded-3xl mt-4 mx-4 md:mx-0">
                <div class="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center text-slate-300 mx-auto mb-3"><i class="fa-solid fa-receipt text-xl"></i></div>
                <h3 class="text-base font-bold text-slate-800 tracking-tight">Sin ventas operativas</h3>
                <p class="text-slate-400 text-sm mt-1 mb-5">Cuando recibas compras, aparecerán aquí agrupadas por fecha.</p>
                <a href="/mis-servicios" class="inline-flex items-center justify-center bg-slate-800 text-white text-xs font-bold py-2 px-5 rounded-full hover:bg-slate-700 transition">
...
function toggleSeleccion(id) {
    ...
    if (seleccionados.has(id)) {
        ...
        uiCircle.classList.add('border-slate-300');
        uiIcon.classList.remove('opacity-100');
        overlay.classList.remove('opacity-100');
    } else {
        seleccionados.add(id);
        uiCircle.classList.remove('border-slate-300');
