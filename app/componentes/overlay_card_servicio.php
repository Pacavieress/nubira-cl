...
//   $ov_show_tutor bool   true (default) | false → oculta avatar+nombre del tutor
//                         (usar false cuando el contexto ya deja claro de quién son todas
//                         las cards, ej. la vitrina personal en perfil.php)
//
...
$ov_liviano     = $ov_liviano ?? false;
$ov_show_tutor  = $ov_show_tutor ?? true;
...
$ov_cls_ring    = $ov_liviano ? 'ring-1 ring-white/40 shadow-[0_1px_3px_rgba(0,0,0,0.15)]' : 'ring-1 ring-white/30';
?>
<!-- [OVERLAY NUBIRA] gradient + avatar tutor + categoría (partial) -->
<div class="absolute inset-0 z-[5] pointer-events-none" style="background:linear-gradient(to bottom, rgba(0,0,0,0.42) 0%, rgba(0,0,0,0.08) 32%, rgba(0,0,0,0) 52%<?= $ov_show_tutor ? ', rgba(0,0,0,0.10) 70%, rgba(0,0,0,0.48) 100%' : '' ?>);"></div>
...
<div class="absolute bottom-3 left-3 z-10 pr-2 flex items-center gap-2 text-white <?= $ov_cls_titulo ?> font-bold" style="max-width:80%; text-shadow:0 1px 3px rgba(0,0,0,0.6);">
        <img src="<?= htmlspecialchars($ov_foto) ?>"
             ...
             class="<?= $ov_cls_avatar ?> rounded-full object-cover <?= $ov_cls_ring ?> avatar-tutor"
             loading="lazy"
