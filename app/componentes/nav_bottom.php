...
$cls_activo   = 'text-[#54A6D8] font-medium';
...
// [NUBIRA 2.0] Caché en archivo: evita 2 queries SQL por cada navegación (5 min TTL).
...
if (!isset($alerta_encendida_php) && !$es_visita_nb && isset($conn)) {
    ...
    $nav_cache = nb_nav_cache_get($uid_nb);
    ...
    if ($nav_cache === null) {
        ...
        nb_nav_cache_set($uid_nb, $nav_cache);
    }

    // Leemos de cache
    $alert_perfil_movil = $nav_cache['alerta'];
    $foto_path_nav  = $nav_cache['foto_path'] ?? '';
    $foto_mtime_nav = $nav_cache['foto_mtime'] ?? 0;
  ...
  <ul class="grid <?= $ocultar_publicar_nb ? 'grid-cols-4' : 'grid-cols-5' ?> text-[11px] text-center pb-1 items-end relative">
    ...
    <li>
        <a href="<?= htmlspecialchars($link_home, ENT_QUOTES, 'UTF-8') ?>" aria-label="Inicio" class="<?= $cls_base ?> <?= $es_inicio ? $cls_activo : $cls_inactivo ?>">
            <div class="w-6 h-6 flex items-center justify-center">
                <?php if ($es_inicio): ?>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6" width="24" height="24" aria-hidden="true">
            ...
            <span class="tracking-[0.01em] leading-none mt-0.5">Inicio</span>
        </a>
    </li>
    ...
    <li>
        <button id="btn-explora" aria-label="Descubrir" class="<?= $cls_base ?> <?= $cls_inactivo ?>">
            ...
            <span class="tracking-[0.01em] leading-none mt-0.5">Descubrir</span>
        </button>
    </li>
    ...
    <li class="relative w-full h-full flex justify-center">
        <button id="btn-publicar"
            ...
            <div class="absolute bottom-3 left-1/2 -translate-x-1/2 w-14 h-14 bg-[#54A6D8] rounded-[18px] flex items-center justify-center text-white z-10 overflow-hidden shadow-md">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-7 h-7 relative z-20" width="28" height="28" aria-hidden="true">
    ...
    <li>
        <a href="<?= htmlspecialchars($link_chat, ENT_QUOTES, 'UTF-8') ?>" aria-label="Mensajes" class="<?= $cls_base ?> <?= $es_mensaje ? $cls_activo : $cls_inactivo ?>">
            ...
            <span class="tracking-[0.01em] leading-none mt-0.5">Mensajes</span>
        </a>
    </li>
    ...
    <li>
        <a href="<?= htmlspecialchars($link_perfil, ENT_QUOTES, 'UTF-8') ?>" aria-label="Perfil" class="<?= $cls_base ?> <?= $es_perfil ? $cls_activo : $cls_inactivo ?>">
            ...
            <span class="tracking-[0.01em] leading-none mt-0.5">Perfil</span>
        </a>
    </li>
