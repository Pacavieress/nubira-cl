...
<main class="pt-16 pb-32 md:pb-12 lg:ml-64 mx-auto max-w-[1000px]">
  <div class="w-full">
    ...
    <div class="sticky top-16 bg-white/95 backdrop-blur-sm z-30 border-b border-slate-100 px-4 md:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-xl md:text-2xl font-extrabold text-slate-900 tracking-tight">Mis Compras</h1>
            <p class="text-slate-400 text-xs font-medium">Historial de tus adquisiciones en Nubira.</p>
        </div>
    </div>
        ...
        <div id="lista-compras" class="pb-10 space-y-2 mt-2">
            ...
            <div class="group-dia space-y-1" id="seccion-apuntes">
                <button onclick="toggleGrupo('content-apuntes', 'icon-apuntes')" class="w-full px-4 md:px-2 pt-4 pb-2 flex items-center justify-between active:bg-slate-50 transition-colors cursor-pointer sticky top-[108px] sm:top-[115px] z-20 bg-white">
                    <div class="flex items-center gap-2">
                        <h2 class="text-xs font-bold text-slate-400 uppercase tracking-widest">Apuntes Comprados (<?= $totalApuntes ?>)</h2>
                    </div>
                ...
                <div id="content-apuntes" class="expand-content collapsed bg-white border-y md:border border-slate-100 md:rounded-2xl">
                    <?php if ($totalApuntes > 0): ?>
                        <ul class="divide-y divide-slate-100">
                            <?php foreach($apuntes as $c): 
                                ...
                                $estadoColor = $aprobado ? 'text-emerald-500 bg-emerald-50' : 'text-amber-500 bg-amber-50';
                            ...
                            <li class="flex items-center justify-between p-4 md:px-4 hover:bg-slate-50 transition-colors gap-3 active:bg-slate-100">
                                <div class="flex items-center gap-3 flex-1 min-w-0 z-20">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-bold text-slate-800 text-[14px] line-clamp-1 leading-tight mb-0.5"><?= htmlspecialchars($c['titulo']) ?></h3>
                                ...
                                <div class="flex flex-col items-end gap-1.5 shrink-0 pl-2 z-20">
                                    ...
                                    <div class="flex justify-end items-center gap-1 bg-slate-100 p-1 rounded-full border border-transparent">
                                        <?php if ($aprobado): ?>
                                            <a href="/ver-apunte?archivo=<?= urlencode($archivo) ?>" target="_blank" class="w-7 h-7 flex items-center justify-center rounded-full text-slate-500 hover:bg-white hover:text-[#54A6D8] active:bg-white transition" title="Descargar/Ver Apunte">
                                                <i class="fa-solid fa-download text-xs"></i>
                                            </a>
                                        <?php else: ?>
                                            <button disabled class="w-7 h-7 flex items-center justify-center rounded-full text-slate-300 cursor-not-allowed" title="Pago Pendiente">
                    ...
                    <?php else: ?>
                        <div class="bg-white p-6 text-center border-b border-slate-100 rounded-2xl">
                            ...
                            <p class="text-sm font-medium text-slate-500">No has comprado apuntes aún.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="group-dia space-y-1 mt-4" id="seccion-servicios">
                <button onclick="toggleGrupo('content-servicios', 'icon-servicios')" class="w-full px-4 md:px-2 pt-4 pb-2 flex items-center justify-between active:bg-slate-50 transition-colors cursor-pointer sticky top-[108px] sm:top-[115px] z-20 bg-white">
                    <div class="flex items-center gap-2">
                        <h2 class="text-xs font-bold text-slate-400 uppercase tracking-widest">Servicios Contratados (<?= $totalServicios ?>)</h2>
                    </div>
                    <i id="icon-servicios" class="fa-solid fa-chevron-down text-slate-400 text-[10px] chevron-icon"></i>
                </button>
                ...
                <div id="content-servicios" class="expand-content collapsed bg-white border-y md:border border-slate-100 md:rounded-2xl">
                    <?php if ($totalServicios > 0): ?>
                        <ul class="divide-y divide-slate-100">
                            <?php foreach($servicios as $s): 
                                $estado = $s['estado'] ?? '';
                                $estilos = [
                                    'pendiente_pago' => 'bg-amber-50 text-amber-500',
                                    'en_progreso' => 'bg-blue-50 text-[#54A6D8]',
                                    'finalizado_vendedor' => 'bg-purple-50 text-purple-500',
                                    'finalizado_comprador' => 'bg-purple-50 text-purple-500',
                                    'liberado' => 'bg-emerald-50 text-emerald-500',
                                ...
                                $clase = $estilos[$estado] ?? 'bg-slate-50 text-slate-500';
                            ...
                            <li class="flex items-center justify-between p-4 md:px-4 hover:bg-slate-50 transition-colors gap-3 active:bg-slate-100">
                                <div class="flex items-center gap-3 flex-1 min-w-0 z-20">
                                    <div class="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 border border-slate-100 relative">
                                        <i class="fa-solid fa-chalkboard-user text-xl text-slate-400"></i>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-1.5 text-[11px] text-slate-400 font-medium truncate">
                                            ...
                                            <span>•</span>
                                            <span class="<?= $clase ?> px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide"><?= htmlspecialchars($texto) ?></span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col items-end gap-1.5 shrink-0 pl-2 z-20">
                                    ...
                                    <div class="flex justify-end items-center gap-1 bg-slate-100 p-1 rounded-full border border-transparent">
                                        <?php if (!$is_cerrado && $estado !== ''): ?>
                                            <a href="/app/mini_aula.php?id=<?= (int)($s['id'] ?? 0) ?>" class="w-7 h-7 flex items-center justify-center rounded-full text-slate-500 hover:bg-white hover:text-emerald-500 active:bg-white transition" title="Ir al Aula">
                                                <i class="fa-solid fa-door-open text-xs"></i>
                                            </a>
                                        <?php else: ?>
                                            <button disabled class="w-7 h-7 flex items-center justify-center rounded-full text-slate-300 cursor-not-allowed" title="Servicio Cerrado">
                                                <i class="fa-solid fa-lock text-xs"></i>
                                            </button>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <?php else: ?>
                        <div class="bg-white p-6 text-center border-b border-slate-100 rounded-2xl">
                            ...
                            <p class="text-sm font-medium text-slate-500">No has contratado servicios aún.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
